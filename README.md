...
- 管理后台：基于 React + Vite、shadcn/ui、tweakcn 和 Tailwind CSS variables，`v1.0.0` 已完成 Riversoft 正式版视觉翻新，`v1.0.1` 已完成第三方 SSO 内部飞书回调兼容补丁，`v1.0.2` 已收口移动端列表、Tabs 和 OAuth 错误页 request id 复制边界，`v1.0.3` 已收口 Base Portal 接入包与完整提示词主流程，`v1.0.4` 已支持第三方 iframe 场景的 OAuth silent SSO。
...
```bash
curl -fsSL https://raw.githubusercontent.com/wodenwang/feishu-iam/v1.0.4/deploy/install.sh | FEISHU_IAM_VERSION=v1.0.4 bash
```
...
`/ready` 应返回 ready，`/version` 应返回 `1.0.4`。
...
注意：`v1.0.4` 继续沿用 `v1.0.0` 的单机 Docker Compose 停机升级模型；如果生产环境 `FEISHU_OAUTH_REDIRECT_URI` 仍是旧 `/api/auth/feishu/callback`，服务会兼容处理，但新部署必须改为 `/oauth/feishu/callback`。
