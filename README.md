...
- 管理后台：基于 React + Vite、shadcn/ui、tweakcn 和 Tailwind CSS variables，`v1.0.0` 已完成 Riversoft 正式版视觉翻新。
...
- 已准备飞书企业自建应用配置。
- 已确认首个平台管理员对应的飞书 `user_id`，`v1.0.0` 初始化只会确保该用户获得 IAM 平台管理员权限。
...
`/ready` 应返回 ready，`/version` 应返回 `1.0.0`。
...
```text
https://github.com/wodenwang/feishu-iam/releases/tag/v1.0.0
```

`v1.0.0` 已完成 amd64 离线 tar 部署和线上 canary。当前线上版本通过以下检查：
...
注意：`v1.0.0` 线上部署使用 amd64 离线 tar；内网 HTTP Registry 多架构 manifest digest 尚未补发。后续恢复 registry push 能力后，再补齐标准多架构镜像信息。
