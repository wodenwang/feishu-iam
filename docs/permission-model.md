...
本文说明 Feishu IAM 的应用与权限模型，并补充管理端权限化后的使用边界，供管理员、开发者和 Agent 做接入、验收和后续迭代参考。
...
`v1.0.5` 起，IAM 角色调整为全局资源，不再业务归属于单个应用。角色与应用通过 `iam_role_applications` 多对多绑定；权限组和权限点仍按应用隔离。角色主体绑定回答“谁拥有这个角色”，按角色全局生效；权限组和权限点绑定回答“这个角色在某个应用下拥有哪些权限”，必须带应用上下文。
...
应用表示一个接入 Feishu IAM 的内部系统，例如 `finance`、`oa`、`crm`。应用拥有全局唯一 `app_key`。权限组和权限点必须归属于某一个应用；IAM 角色是全局资源，通过角色-应用绑定决定它在哪些应用中生效。
...
IAM 角色是 Feishu IAM 内部的授权载体，不依赖飞书角色或飞书用户组。`v1.0.5` 起，角色是全局资源，可以绑定多个应用；角色主体绑定在角色级维护，权限组和直接权限点绑定在角色 + 应用上下文中维护。
...
角色 key 不强制带 `app_key` 前缀。管理端在应用上下文中读取角色时，会同时返回当前 `app_key`、关联应用摘要和角色 `key`，避免跨应用阅读时产生歧义。
...
4. 找出已绑定当前应用且角色和绑定关系均 active 的 IAM 角色。
5. 命中绑定到该用户的角色。
6. 命中绑定到用户直接部门的角色。
7. 丢弃 orphaned 主体绑定。
...
绑定角色主体。主体绑定在角色级生效，会影响该角色关联的所有应用：
...
1. 查看应用列表。
2. 点击应用后查看该应用的权限组、权限点和权限资产对比；角色新增、编辑、启停和配置统一从 `权限管理` 进入。
