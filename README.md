...
- 管理后台：基于 React + Vite、shadcn/ui、tweakcn 和 Tailwind CSS variables，`v1.0.0` 已完成 Riversoft 正式版视觉翻新，`v1.0.1` 已完成第三方 SSO 内部飞书回调兼容补丁，`v1.0.2` 已收口移动端列表、Tabs 和 OAuth 错误页 request id 复制边界，`v1.0.3` 已收口 Base Portal 接入包与完整提示词主流程，`v1.0.4` 已支持第三方 iframe 场景的 OAuth silent SSO，`v1.0.5` 已完成权限管理角色配置工作台重构，`v1.0.6` 已收口权限管理工作台 UI/UX 小版本。
...
`v1.0.6` GitHub Release 创建后，正式版本部署使用 one-liner 下载部署文件：

```bash
curl -fsSL https://raw.githubusercontent.com/wodenwang/feishu-iam/v1.0.6/deploy/install.sh | FEISHU_IAM_VERSION=v1.0.6 bash
```
...
```bash
cd ~/feishu-iam
FEISHU_IAM_IMAGE_TAG=v1.0.6 APP_VERSION=1.0.6 ./upgrade.sh
```
...
`/ready` 应返回 ready，`/version` 应返回 `1.0.6`。
...
说明：`v1.0.6` GitHub Release、镜像上传和生产部署仍待授权完成；在发布动作完成前，线上稳定版本仍以实际 `/version` 读回为准。
...
GitHub Release 待创建地址：

```text
https://github.com/wodenwang/feishu-iam/releases/tag/v1.0.6
```

`v1.0.6` 是权限管理 UI/UX 小版本，基于 `v1.0.5` 的角色配置工作台继续收口布局和交互质量。它不新增 DDL，不改变权限模型或权限计算，只修复生产标注和浏览器调研发现的前端体验问题。`v1.0.6` 已通过以下本地和浏览器检查：

- `pnpm --filter @feishu-iam/admin-web test -- src/features/permissions/PermissionRoleDetailSheet.test.tsx src/features/org-browser/org-user-selector.test.tsx` 通过。
...
- `git diff --check` 通过。
- Playwright 浏览器审计通过：`output/playwright/v1.0.6-permission-uiux-audit/uiux-audit.json` 覆盖 5 条权限管理路径 x 3 个视口，`results=15`、`consoleErrors=0`、`failedRequests=0`、`failures=0`。
- 权限管理 `组织与用户` 待选区不再被右侧已选区撑高。
- 权限管理 `应用权限` 可选权限组区不再被右侧绑定结果预览撑高。
- 当前应用切换已改为纵向 tab，不再使用下拉切换。
- 角色配置工作台标题层级去重，避免重复展示 `角色配置工作台`。
- `绑定结果预览` 和权限点对比具备稳定滚动边界。

`v1.0.5` 已完成以下正式版能力，`v1.0.6` 不回退：
...
注意：`v1.0.6` 继续沿用 `v1.0.0` 的单机 Docker Compose 停机升级模型；如果生产环境 `FEISHU_OAUTH_REDIRECT_URI` 仍是旧 `/api/auth/feishu/callback`，服务会兼容处理，但新部署必须改为 `/oauth/feishu/callback`。
...
- 已完成本地 focused tests、typecheck、lint、build、`git diff --check` 和 Playwright 三视口审计；GitHub Release、镜像上传和生产部署仍待授权。
