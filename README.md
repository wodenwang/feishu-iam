...
```bash
curl -fsSL https://raw.githubusercontent.com/wodenwang/feishu-iam/v1.0.1/deploy/install.sh | FEISHU_IAM_VERSION=v1.0.1 bash
```
...
```bash
cd ~/feishu-iam
FEISHU_IAM_IMAGE_TAG=v1.0.1 APP_VERSION=1.0.1 ./upgrade.sh
```
...
`/ready` 应返回 ready，`/version` 应返回 `1.0.1`。
...
注意：`v1.0.1` 继续沿用 `v1.0.0` 的单机 Docker Compose 停机升级模型；如果生产环境 `FEISHU_OAUTH_REDIRECT_URI` 仍是旧 `/api/auth/feishu/callback`，服务会兼容处理，但新部署必须改为 `/oauth/feishu/callback`。
